...
class DependentFilteredEntityController extends Controller
{
    ...
    public function getOptionsAction()
    {
        ...
        if ($entity_inf['many_to_many']) {
            $qb
                ->join($queryRootAlias.'.'.$entity_inf['parent_property'], $entity_inf['parent_property'])
                ->where($entity_inf['parent_property'].'.id' . ' IN (:parent_id)')
            ;
        } else {
            $qb->where($queryRootAlias.'.'.$entity_inf['parent_property'] . ' = :parent_id');
        }
	...
	if(!empty($entity_inf['table_name'])){
	    $qra = 't';
	    $qb1 = $em->createQueryBuilder()
            	->select('IDENTITY('.$qra.'.'.$entity_inf['column_name'].')')
            	->from($entity_inf['table_name'], $qra)
		->where($qra.'.id IN (:ids)')
                ->setParameter('ids', $parent_id)
            	->getQuery()
	    	->getResult();
        ...
        $qb = $em->createQueryBuilder()
            ->select('DISTINCT '.$queryRootAlias)
...
//        }
        ...
        $getter =  $this->getGetterName($entity_inf['result_property']);
